...
\begin{tikzpicture}
  \drawstruct{(0,0)}
  \llstructcell{0.5}{padding}{\texttt{<main>}}
  ...
  \structcell[pointer]{~} \coordinate (mainHead) at (currentcell.north);

  \llstructcell{0.5}{padding}{\texttt{p}}
  \structcell[staticLink]{staticLink} \coordinate (p1sl) at (currentcell.west);
  \structcell[dynamicLink]{dynamicLink}  \coordinate (p1dl) at (currentcell.east);
  \structcell[freecell]{level = 1}
  \structcell[freecell]{procedure = \texttt{p}}
  \structcell[freecell]{entries = [10]}
  \structcell[pointer]{~} \coordinate (p1Head) at (currentcell.north);
  ...
  \structcell[pointer]{~} \coordinate (p2Head) at (currentcell.north);

  \llstructcell{0.5}{padding}{\texttt{q}}
  \structcell[staticLink]{staticLink} \coordinate (qsl) at (currentcell.west);
  \structcell[dynamicLink]{dynamicLink} \coordinate (qdl) at (currentcell.east);
  \structcell[freecell]{level = 2}
  \structcell[freecell]{procedure = \texttt{q}}
  \structcell[freecell]{entries = []}

  \draw[->] (p1sl) to [out=150] (mainHead);
  \draw[->] (p1dl) to [in=70] (mainHead);
  \draw[->] (p2sl) to [out=150] (mainHead);
  \draw[->] (p2dl) to [in=70] (p1Head);
  \draw[->] (qsl) to [out=150] (p2Head);
  \draw[->] (qdl) to [in=70] (p2Head);
\end{tikzpicture}
\rule{\columnwidth}{0.5pt}
